...
def backbrute(web):
    ...
    print(
        f'{C} [*] Path to file to be used {O}(Default: files/fuzz-db/backdoor_paths.lst)...{C}'
    )
    ...
    try:
        if ul := check0x00(web, mo, gen_headers):
            print(f'{G} [+] The following possible backdoors were found!{C}{color.TR2}{C}')
            for u in ul:
                print(f'{O} [+] Path to backdoor :{C}{color.TR3}{C}{G}{u}{C}{color.TR2}{C}')
                save_data(database, module, lvl1, lvl2, lvl3, name, u)
        else:
            print(f'{R} [-] No backdoors were found!')
